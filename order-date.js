...
function sortOrders(event) {
  ...
  if (sortBy === "orderDate") {
    ...
    rowArray.forEach((r, i) => {
      if (i > 0 && i < orderList.length + 1) table.deleteRow(1);
    });
...
function renderRow(newOrder, orderIndex) {
  orderIndex === undefined ? orderList.length : orderIndex;
  ...
  Object.keys(newOrder).forEach(key => {
    ...
    if (key === sortByField) currentCell.style.backgroundColor = "red";
    cellIndex++;
  });
  currentCell = row.insertCell(cellIndex++);
  ...
  if (orderList.length === 1) {
    ...
    updateSummaryRow(
      orderList.reduce((prev, curr) => prev + Number(curr.totalAll), 0)
    );
  }
}
